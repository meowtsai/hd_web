...
exports.preregister = function(req, res, next) {
  ...
  const gift_data = [
    {
      id: 1,
      title: "伐木大師的饋贈",
      items: ["【石幣】*50,000", "【伐木斧 · 2级】*1 ", "【海民耐吃肉乾】 *3"]
    },
    {
      id: 2,
      title: "挖礦大師的饋贈",
  ...
  const meta_desc =
    "《海島紀元》採用了風格化的卡通渲染，人物設定突出搞怪風趣。在傳統角色扮演的基礎上，遊戲還將採集製作、島嶼建設、寵物馴養、元素互動等玩法融入其中。玩家可以砍樹、採礦、釣魚、種菜，建設管理屬於自己的小島，製作豐富的道具和美食，還可以在野外捕捉各具特色的寵物，帶著他們一起冒險。此外，遊戲中還加入了元素互動，增加了戰鬥的可玩性和策略性，玩家可以通過靈活掌控水、火、毒、風等元素間的相生相剋，改變戰局。";
...
exports.post_preregister = [
  ...
  body("chk_agree")
    .equals("true")
    .withMessage("請同意資料條款及隱私權政策"),
  sanitizeBody("email_address")
    .trim()
    .escape(),
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      var selQuery = connection.query(
        "SELECT email,create_time  FROM event_preregister where event_id=? and email=?",
        [16, email],
        function(error, results, fields) {
          ...
          if (results == undefined || results.length == 0) {
            var register_data = {
              event_id: 16,
              nick_name: ipInfo.city,
              email: email,
              ip: req.headers["x-forwarded-for"],
              country: ipInfo.country
            };
            var query = connection.query(
              "INSERT INTO event_preregister SET ?",
              register_data,
              function(error, xresults, fields) {
                ...
                if (process.env.NODE_ENV != "development") {
                  ...
                  let html_template = fs.readFileSync(
                    __dirname + "/../public/template/mail.txt",
                    "utf8"
                  );

                  let mailOptions = {
                    from:
                      '"🌊 海島紀元遊戲營運團隊 🍖 " <no-reply@longeplay.com.tw>', // sender address
            ...
            return res.status(200).json({
              ...
              registered: {
                ...
                create_time: moment(results[0].create_time).format(
                  "YYYY-MM-DD HH:mm:ss"
                )
              }
            });
