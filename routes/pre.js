...
router.get('/preregister_mb9999', function(req, res, next) {
    ...
    res.render('pre/preregister_mb', { title: '海島紀元事前登錄', gift_data:gift_data,meta_keyword:"海島紀元",meta_desc:"《海島紀元》採用了風格化的卡通渲染，人物設定突出搞怪風趣。在傳統角色扮演的基礎上，遊戲還將採集製作、島嶼建設、寵物馴養、元素互動等玩法融入其中。玩家可以砍樹、採礦、釣魚、種菜，建設管理屬於自己的小島，製作豐富的道具和美食，還可以在野外捕捉各具特色的寵物，帶著他們一起冒險。此外，遊戲中還加入了元素互動，增加了戰鬥的可玩性和策略性，玩家可以通過靈活掌控水、火、毒、風等元素間的相生相剋，改變戰局。" });    
});


router.get('/preregister', function(req, res, next) {
    res.render('pre/preregister', { title: 'YOU ARE NOT USING MOBILE' });    
});
